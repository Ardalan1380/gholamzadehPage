...
function ShowingResultPage({data}) {
    if(!data == null || data) {
      ...
      return (
        ...
        <div className={`border-2 border-[#474545] w-[380px] min-h-[500px] flex flex-col rounded-2xl bg-[#474545]`}>
            <div className={`flex justify-center items-center w-[380px] mx-auto min-h-[400px] flex-col`}>
                        <p className={`text-white text-[19px] font-semibold`}>شما پیش ثبت نامی انجامی نداده اید</p>
                        <button onClick={() => router.push("/PreRegistration")} className={`mt-[15px] bg-[#C38065] p-3 text-[16px] text-white rounded-lg flex justify-center items-center`}>
                            پیش ثبت نام خودرو
                            <PiNotepadLight className={`text-[18px]`} />
                            </button>
                </div>
        </div>
